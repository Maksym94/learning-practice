...
public class AccountInfoSpring implements AccountInfo{
	...
	public List<Account> getAccountInfo(String login) {
		...
		List<Account> list =  (List<Account>) template.findByCriteria(DetachedCriteria
	    		.forClass(Account.class).add(Restrictions.eq("loginAccount", login))
	...
	public boolean putMoney(String login, BigDecimal amount) {
		...
		template.find("select balance from Account "
				+ "where loginAccount=? ORDER BY idAccount DESC", login);
	...
	public boolean removeMoney(String login, BigDecimal amount) {
		...
		@SuppressWarnings("unchecked")
		List<BigDecimal> oneResultOfMoney= (List<BigDecimal>)  template.find("select balance from Account "
				+ "where loginAccount=? order by idAccount DESC", login);
